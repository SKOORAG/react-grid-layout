...
var GridItem = /*#__PURE__*/function (_React$Component) {
  ...
  _createClass(GridItem, [{
    ...
    /*: string*/
      ...
      var handleOrientation = node.classList.item(1).indexOf("w") != -1 ? "west" : "east"; // Get new XY

      var _calcWH = (0, _calculateUtils.calcWH)(this.getPositionParams(), size.width, size.height, x, y, handleOrientation),
          w = _calcWH.w,
      ...
      if (elem) {
        ...
        if (useCSSTransforms) {
          var transformCSS = elem.style.transform.replace(/[^\d.,]/g, "").split(",");
        ...
        if (["react-resizable-handle-sw", "react-resizable-handle-w", "react-resizable-handle-nw", "react-resizable-handle-n", "react-resizable-handle-ne"].indexOf(node.classList.item(1)) === -1) {
          this.setState({
            resizing: handlerName === "onResizeStop" ? null : size
          });
        } else {
          if (["react-resizable-handle-sw", "react-resizable-handle-w"].indexOf(node.classList.item(1)) !== -1) {
            size.left = currentLeft - (size.width - currentWidth);
            size.top = currentTop;
          } else if (["react-resizable-handle-n", "react-resizable-handle-ne"].indexOf(node.classList.item(1)) !== -1) {
